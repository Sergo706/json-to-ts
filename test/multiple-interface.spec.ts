...
describe("Multiple interfaces", function () {
  ...
  b: number;
    ...
    interfaces.forEach((typeInterface) => {
      assert.ok(expectedTypes.includes(typeInterface.trim()));
    });
  });

  it("should have unique names for nested objects", function () {
    const json = {
      name: "Larry",
      parent: {
        name: "Garry",
  ...
  it("should not singularize if not array type (cats: {} => cats: Cats )", function () {
    const json = {
      cats: {
